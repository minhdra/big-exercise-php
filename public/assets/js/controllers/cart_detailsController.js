const nameController = 'cart_details/';
const nameSelf = 'cart_detail/';

app.controller('cart_detailsController', cart_detailsController);
function cart_detailsController($scope, $http) {
  $scope.title = 'Cart Details';
  $scope.currentPage = 1;
  $scope.pageSize = 10;
  $scope.keyword = '';
  $scope.serial = 1;
  ...
  $scope.openModal = (id) => {
    ...
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a cart detail';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a cart detail';
      $http({
        method: 'GET',
