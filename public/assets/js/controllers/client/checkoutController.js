...
function checkoutController($rootScope, $scope, $http) {
  ...
  $scope.loadData = () => {
    if ($scope.customer.username) {
      connect_api(
        'GET',
        apiBase + nameCustomer + $scope.customer.id,
        null,
        function (res) {
          $scope.customer = res.data;
          $scope.countTotal();
          // console.log($scope.customer);
          if ($scope.customer.cart_details.length === 0)
            location.href = '/shop';
          if ($scope.customer.delivery_addresses.length > 0) {
            $scope.delivery_address = $scope.customer.delivery_addresses.filter(
              (item) => item.status === 1
            )[0];
          }
        }
  ...
  $scope.updateInfo = (info) => {
    ...
    if (info.id) {
      connect_api(
        'PUT',
        apiBase + nameCustomerInfo + info.id,
        info,
        function (res) {}
      );
    } else {
  ...
  $scope.saveDeliveryAddress = () => {
    ...
    if ($scope.isCreate) {
      ...
      connect_api(
        'PUT',
        apiBase + nameDeliveryAddress + $scope.address_id,
        delivery,
        function (res) {
          if ($scope.delivery_address.status === 1)
            $scope.delivery_address = res.data;
  ...
  $scope.changeDefault = (item) => {
    ...
    connect_api(
      'PUT',
      apiBase + nameDeliveryAddress + item.id,
      item,
      function (res) {
        $scope.loadData();
      }
  ...
  $scope.payment = () => {
    const delivery_address =
      [
        $scope.delivery_address.full_name,
        $scope.delivery_address.phone_number,
      ].join(', ') +
      ' - ' +
      [
        $scope.delivery_address.specific_address,
        $scope.delivery_address.commune,
        $scope.delivery_address.district,
        $scope.delivery_address.province,
      ].join(', ');
    const order = {
      delivery_address,
      customer_id: $scope.customer.id,
    ...
    connect_api('POST', apiBase + nameOrder, order, function (res) {
      ...
      connect_api(
        'DELETE',
        apiBase + 'carts/' + $scope.customer.cart.id,
        null,
        function () {}
      );
    });
