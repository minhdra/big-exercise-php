...
function colorsController($scope, $http) {
  ...
  // Get all categories
  const loadData = () => {
    connect_api('GET', apiBase + nameColors, null, function (res) {
      $scope.data = res.data.colors;
  ...
  $scope.openModal = (id) => {
    ...
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a color';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a color';
      $http({
        method: 'GET',
        url: apiBase + nameColors + id,
      }).then(
        (res) => {
  ...
  $scope.saveData = () => {
    ...
    if ($scope.id == 0)
    {
      connect_api('POST', apiBase + nameColors, $scope.item, function (res) {
        $scope.data = [res.data, ...$scope.data];
        $('#large').modal('hide');
      })
    } else
    { // is update
      connect_api('PUT', apiBase + nameColors + $scope.id, $scope.item, function (res) {
  ...
  $scope.remove = (id) => {
    ...
    {
      connect_api('DELETE', apiBase + nameColors + id, null, function (res) {
