const nameController = 'admin_infos/';
const nameSelf = 'admin_info/';

app.controller('admin_infosController', admin_infosController);
function admin_infosController($scope, $http) {
  $scope.title = 'Admin info';
  $scope.currentPage = 1;
  $scope.pageSize = 10;
  $scope.keyword = '';
  $scope.serial = 1;
  ...
  $scope.openModal = (id) => {
    ...
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a admin information';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a admin information';
      $http({
        method: 'GET',
