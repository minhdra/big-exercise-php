...
app.controller('sizesController', sizesController);
function sizesController($scope, $http) {
  $scope.title = 'Sizes';
  $scope.currentPage = 1;
  $scope.pageSize = 10;
  $scope.keyword = '';
  ...
  // Get all categories
  const loadData = () => {
    connect_api('GET', apiBase + nameSize, null, function (res) {
      $scope.data = res.data.sizes;
  ...
  $scope.openModal = (id) => {
    ...
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a size';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a size';
      $http({
        method: 'GET',
        url: apiBase + nameSize + id,
      }).then(
        (res) => {
  ...
  $scope.saveData = () => {
    ...
    if ($scope.id == 0)
    {
      connect_api('POST', apiBase + nameSize, $scope.item, function (res) {
        $scope.data = [res.data, ...$scope.data];
        $('#large').modal('hide');
      })
    } else
    { // is update
      connect_api('PUT', apiBase + nameSize + $scope.id, $scope.item, function (res) {
  ...
  $scope.remove = (id) => {
    ...
    {
      connect_api('DELETE', apiBase + nameSize + id, null, function (res) {
