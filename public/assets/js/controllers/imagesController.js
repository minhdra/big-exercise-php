...
function imagesController($scope, $http) {
  ...
  // Get all categories
  const loadData = () => {
    connect_api('GET', apiBase + nameImage, null, function (res) {
  ...
  $scope.openModal = (id) => {
    ...
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a image';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a image';
      $http({
        method: 'GET',
        url: apiBase + nameImage + id,
      }).then(
        (res) => {
  ...
  $scope.saveData = () => {
    ...
    if ($scope.id == 0)
    {
      connect_api('POST', apiBase + nameImage, $scope.item, function (res) {
        $scope.data = [res.data, ...$scope.data];
        $('#large').modal('hide');
      })
    } else
    { // is update
      connect_api('PUT', apiBase + nameImage + $scope.id, $scope.item, function (res) {
  ...
  $scope.remove = (id) => {
    ...
    {
      connect_api('DELETE', apiBase + nameImage + id, null, function (res) {
