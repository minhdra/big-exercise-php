const nameController = 'customer_infos/';
const nameSelf = 'customer_info/';

app.controller('customer_infosController', customer_infosController);
function customer_infosController($scope, $http) {
  ...
  // open the modal in customer
  $scope.openModal = (id) => {
    $scope.id = id;
    // Insert
    if (id == 0) {
      $scope.modalTitle = 'Insert a customer information';
      $scope.item = null;
    } else { // Edit
      $scope.modalTitle = 'Edit a customer information';
      $http({
        method: 'GET',
